# ML Model Evaluation Report — v3
## Trading System Automation — Next-Day Return Prediction

**Date:** March 2026
**Dataset:** SimFin bulk CSV — 30 US large-cap tickers, 2020–2024
**Evaluation scope:** Per-ticker prototype (sections 2–3) + Production pooled models v2 (section 6) + Signal threshold fix (section 11)
**Changes from v2:** ~~100% HOLD signal issue~~ resolved — percentile-based dynamic thresholds implemented and deployed. Finding 1 and Limitations table updated to reflect fix. Section 11 added.
**Source:** `notebooks/ml_exploration.ipynb` (per-ticker sections 5–6) + `python model/train.py --all` (production v2) + `model/trained/thresholds.json` (v3 threshold fix)
...
|---|---|
| > buy threshold (p75 of predictions) | **BUY** |
| < sell threshold (p25 of predictions) | **SELL** |
| between p25 and p75 | **HOLD** |

> **v3 change:** The fixed ±0.5% / ±2.0% 5-tier thresholds are replaced by dynamic percentile-based thresholds. See Section 11 for full rationale and implementation details.
...
### 6.5 Signal-bin breakdown — v2 raw (pre-threshold fix)

> **Historical record.** These are the signal distributions observed before the dynamic threshold fix (Section 11) was applied. 100% STAY was the root cause that triggered the v3 fix.

#### Standard pool (pre-fix)
...
> **Root cause:** Ridge with strong regularisation (α=10–100) shrinks all coefficients toward zero, collapsing predicted returns into the narrow ±0.5% HOLD window. The fixed ±0.5% / ±2.0% thresholds were designed for models with higher prediction variance. See Section 11 for the fix.

### 6.6 v1 vs. v2 vs. v3 — Key metric changes

| Metric | v1 (OLS) | v2 (Ridge α=10/100) | v3 (Ridge + dynamic thresholds) | Δ v2→v3 |
|---|---|---|---|---|
| Standard R² | −0.0026 | **−0.0021** | −0.0021 (unchanged) | — |
| Standard MAE | 0.01530 | **0.01496** | 0.01496 (unchanged) | — |
| Standard RMSE | — | 0.02344 | 0.02344 (unchanged) | — |
| Fallback R² | −0.0025 | **−0.0012** | −0.0012 (unchanged) | — |
| Fallback MAE | 0.00996 | **0.00995** | 0.00995 (unchanged) | — |
| Direction acc | not logged | 50.67% / 51.44% | 50.67% / 51.44% (unchanged) | — |
| Signal distribution | not logged | **100% STAY** | **~25% BUY / ~50% HOLD / ~25% SELL** | ✅ Fixed |
| Signal thresholds | hardcoded ±0.5%/±2% | hardcoded ±0.5%/±2% | **percentile-based (p75/p25)** | ✅ Fixed |

> **v3 does not change model weights.** The Ridge models are identical to v2. Only the signal conversion logic was updated. Regression metrics are therefore unchanged.

### 6.7 Hyperparameter configuration (v2, unchanged in v3)
...
| min_child_samples | **30** | Minimum leaf size — key regulariser |
...
#### Summary: overfitting risk ranking
...
| 6 | 605 | +0.000881 | −0.000090 | 0.5041 |
...
## 8. Context: Per-Ticker Prototype vs. Production Pooled (v1 vs. v2 vs. v3)

| | Per-ticker prototype | Production v1 (OLS) | Production v2 (Ridge+LGBM) | Production v3 (dynamic thresholds) |
|---|---|---|---|---|
| Training rows | ~900 per model | 22,148 / 4,860 | **24,201 / 4,860** | 24,201 / 4,860 (unchanged) |
| Models evaluated | LR, RF, GBR | LR, RF, GBR | **Ridge(CV), RF, GBR, LightGBM** | Same as v2 |
| Alpha selection | N/A | N/A | **TimeSeriesSplit CV, dir. acc.** | Same as v2 |
| Winner | LR (13/25 tickers) | LinearRegression | **Ridge α=10 / α=100** | Same as v2 |
| Winner R² | Mean −0.67 | −0.0026 / −0.0025 | **−0.0021 / −0.0012** | Same as v2 |
| Winner MAE | 0.0175 (mean) | 0.01530 / 0.00996 | **0.01496 / 0.00995** | Same as v2 |
| Direction accuracy | ~49–50% (per-ticker) | Not logged | **50.67% / 51.44%** | Same as v2 |
| Signal bins | Not logged | Not logged | **100% STAY (all bins)** | **~25% BUY / ~50% HOLD / ~25% SELL** ✅ |
| Signal thresholds | N/A | hardcoded ±0.5%/±2% | hardcoded ±0.5%/±2% | **percentile-based (p75/p25)** ✅ |
| Calibration | Not performed | Not performed | **Computed — plots generated** | Same as v2 |
| RMSE | Logged | Not logged | **Logged** | Same as v2 |

---

## 9. Key Findings & Implications for the Trading System

### ~~Finding 1 — The model produces only HOLD signals~~ → **RESOLVED in v3**

> This finding was present in v2. It has been addressed. See Section 11 for the full fix.

~~All predictions cluster in the STAY bin (|pred| < 0.5%). The Streamlit app will generate only HOLD signals in live inference.~~

**v3 status:** Dynamic percentile-based thresholds are now in production. The app generates ~25% BUY, ~50% HOLD, ~25% SELL signals by construction. The thresholds adapt to the actual prediction distribution of each pool rather than depending on fixed ±0.5% / ±2.0% hardcoded values.
...
|---|---|---|
| ~~100% HOLD signals in production~~ | ~~High~~ | ✅ **Resolved in v3** — dynamic percentile thresholds (p75/p25) now produce ~25% BUY / ~50% HOLD / ~25% SELL. See Section 11. |
...
1. ~~**Recalibrate signal thresholds**~~ ✅ **Done in v3** — dynamic p75/p25 thresholds deployed via `model/trained/thresholds.json`. See Section 11.
...
## 11. v3 Fix: Percentile-Based Dynamic Signal Thresholds

### 11.1 Problem statement

Ridge regression with strong L2 regularisation (α=10–100) shrinks all model coefficients toward zero. For a target with near-zero expected value (daily stock returns average ~0.04% with ~1–2% daily volatility), this means all predictions fall within a narrow band around zero — well inside the original fixed ±0.5% HOLD threshold. The result: **100% HOLD signals** regardless of which ticker or which trading day.

This is not a model bug. It is a mismatch between the model's prediction scale and the hardcoded threshold scale. The fix must adapt the thresholds to the model's actual prediction range, not the other way around.

### 11.2 Fix design: percentile-based thresholds

**Principle:** Instead of asking "does the predicted return exceed a fixed ±0.5%?", ask "is this prediction among the model's most positive or most negative predictions relative to its own history?"

**Implementation:**

```
buy threshold  = 75th percentile of all predictions across the full dataset
sell threshold = 25th percentile of all predictions across the full dataset
```

This guarantees:
- The top 25% of predictions → **BUY**
- The bottom 25% of predictions → **SELL**
- The middle 50% of predictions → **HOLD**

The thresholds are **pool-specific**: computed separately for the standard pool (25 tickers) and the fallback pool (BAC/GS/JPM/MA/V), since the two Ridge models have different coefficient magnitudes (α=10 vs α=100).

### 11.3 Computed threshold values

Thresholds computed by `compute_and_save_thresholds()` in `model/train.py` and stored in `model/trained/thresholds.json`:

| Pool | Buy threshold (p75) | Sell threshold (p25) | Interpretation |
|---|---|---|---|
| Standard (25 tickers, Ridge α=10) | **+0.1427%** | **+0.0389%** | Predictions above +0.1427% → BUY; below +0.0389% → SELL |
| Fallback (5 tickers, Ridge α=100) | **+0.1499%** | **−0.0344%** | Predictions above +0.1499% → BUY; below −0.0344% → SELL |

> **Important observation — standard pool:** Both the buy and sell thresholds are positive (+0.1427% and +0.0389%). This means even a mildly positive predicted return can trigger a SELL signal if it falls below the 25th percentile of the prediction distribution. This is a consequence of the equity risk premium: Ridge slightly captures the positive drift in stock prices, shifting the entire prediction distribution above zero. The p25 threshold therefore represents "weaker than typical positive prediction" rather than "expected negative return."

> **Fallback pool:** The sell threshold is slightly negative (−0.0344%), indicating that the fallback model does produce a small fraction of genuine negative predictions. For financial stocks, the Ridge model with stronger regularisation (α=100) is slightly less biased toward positive predictions.
...
| File | Change |
|---|---|
| `model/train.py` | Added `compute_and_save_thresholds()` — called automatically at end of `--all` run; computes p75/p25 across all ticker predictions and writes `thresholds.json` |
| `model/strategy.py` | Updated `prediction_to_signal(prediction, thresholds=None)` — accepts optional `thresholds` dict; falls back to legacy integer convention if `None` |
| `app/pages/go_live.py` | Loads `thresholds.json` at startup; passes pool-appropriate thresholds to `prediction_to_signal()` at inference |
| `app/pages/backtesting.py` | Same — thresholds loaded at startup and applied in backtest simulation loop |
| `model/trained/thresholds.json` | New file — stores the computed thresholds; deployed alongside the `.pkl` model files |

**Signal conversion logic (updated `prediction_to_signal`):**

```python
def prediction_to_signal(prediction, thresholds=None):
    if thresholds is not None:
        if prediction > thresholds["buy"]:
            ...
            return Signal.SELL
        return Signal.HOLD
    # Legacy fallback if thresholds.json is missing
    if prediction == 1: return Signal.BUY
    if prediction == -1: return Signal.SELL
    return Signal.HOLD
```

### 11.5 Signal distribution after fix

With percentile-based thresholds, the expected signal distribution is ~25% BUY, ~50% HOLD, ~25% SELL by construction (since thresholds are the exact p75/p25 of the prediction distribution). The precise distribution on the test set will vary slightly due to ties and discrete quantile boundaries, but will be close to this split.

### 11.6 Design trade-offs

| Aspect | Assessment |
|---|---|
| **Guarantees signal diversity** | Yes — by construction, the model always generates BUY and SELL signals. The app is never all-HOLD. |
| **Signal represents absolute conviction** | No — SELL does not necessarily mean the model predicts a price drop. It means the model is less bullish than usual. |
| **Adapts to model output scale** | Yes — thresholds scale automatically with the model's prediction range. If the model is retrained with different alpha or features, rerunning `--all` recomputes the thresholds. |
| **Consistent with calibration analysis** | Partially — the top decile of the fallback pool has genuine directional signal (63.9% dir. acc). BUY signals from the fallback pool are more likely to correspond to real upward moves than BUY signals from the standard pool. |
| **Look-ahead in threshold computation** | Mild — thresholds are computed from all rows (train + test). For a production system, thresholds should be computed from training rows only. In this course context the effect is negligible. |

---

*Report v3 generated March 2026. Models: `model/trained/model_pooled.pkl` (Ridge α=10.0, 25 tickers) and `model/trained/model_pooled_fallback.pkl` (Ridge α=100.0, 5 tickers). Thresholds: `model/trained/thresholds.json` (buy: +0.1427% std / +0.1499% fb; sell: +0.0389% std / −0.0344% fb). Calibration plots: `model/trained/calibration_standard.png` and `model/trained/calibration_fallback.png`.*
